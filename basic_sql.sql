...
SELECT EmployeeID, (FirstName+' '+LastName) AS NAME, YEAR(GETDATE())-YEAR(BirthDate) AS AGE FROM Employees
...
SELECT ProductID, ProductName, UnitPrice, UnitsInStock, CategoryID FROM Products ORDER BY CategoryID ASC, UnitsInStock DESC
...
SELECT COUNT(*) AS COUNT FROM Orders WHERE ShipCountry = 'France' AND YEAR(ShippedDate) = '1997'
--8. แสดงราคาต่อหน่วยของสินค้าที่แพงสุด และถูกที่สุด
SELECT MIN(UnitPrice) AS MIN, MAX(UnitPrice) AS MAX FROM Products
SELECT * FROM Products WHERE UnitPrice = (SELECT MAX(UnitPrice) FROM Products)
--9. จงแสดงอายุของพนักงานที่มากสุดและอายุน้อยสุด
SELECT MAX(YEAR(GETDATE()) - YEAR(BirthDate)) AS MaxAge, MIN(YEAR(GETDATE())-YEAR(BirthDate)) AS MinAge FROM Employees
--10. แสดงรหัสสินค้า ราคาต่อหน่วย จำนวนที่ซื้อ ราคารวม ของรายการสั่งซื้อที่อยู่ในใบสั่งซื้อหมายเลข10248
SELECT ProductID, UnitPrice, Quantity, (UnitPrice*Quantity)AS SumPrice FROM [Order Details] WHERE OrderID = '10248'
--11. แสดงยอดสั่งซื้อรวมของใบสั่งซื้อหมายเลข 10248
SELECT SUM(UnitPrice * Quantity) AS TotalPrice FROM [Order Details] WHERE OrderID = 10248
--12. แสดงอายุเฉลี่ยของพนักงาน
SELECT AVG(YEAR(GETDATE())-YEAR(BirthDate)) AS AvgAge FROM Employees
--13. แสดงรหัสประเภทสินค้าและจำนวนรายการสินค้าในแต่ละประเภท
SELECT CategoryID, COUNT(ProductID) as NumProducts FROM Products GROUP BY CategoryID
